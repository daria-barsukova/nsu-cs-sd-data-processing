...
(defn notify-msg
  ...
  (let [bill-requirements (state :bill)
         current-buffer (state :buffer)
         required-amount (min (- (bill-requirements ware) (current-buffer ware)) amount)
         updated-buffer
         (if (>= (bill-requirements ware) (current-buffer ware))
           (try
             #_{:clj-kondo/ignore [:redundant-do]}
             (do
               (swap! storage-atom #(- % required-amount))
               (update current-buffer ware #(+ % required-amount)))
             (catch IllegalStateException _ current-buffer))
           current-buffer)]
     (if (= bill-requirements updated-buffer)
       (do
         (Thread/sleep (state :duration))
         (send ((state :target-storage) :worker) supply-msg (state :amount))
         (assoc state :buffer (reduce-kv (fn [acc key _] (assoc acc key 0)) {} bill-requirements)))
